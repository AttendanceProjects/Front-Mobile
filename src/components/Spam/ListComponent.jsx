...
export const ListComponent = ({ load, image, size, name, role, date, startTime, message, action, type, startIssues, id, daily }) => {
  const [ issueM, setIssueM ] = useState( false );
  const [ issues, setIssues ] = useState( false );

  return (
    <>
      <View style={{ borderWidth: 1, width: '99%', borderColor: '#90b8f8', backgroundColor: '#90b8f8', height: 150, marginTop: 5, padding: 10, alignItems: 'center',justifyContent: 'space-around', borderRadius: 20 }}>
        {
          load
            ? <LoadingComponent
                color={ 'blue' }
